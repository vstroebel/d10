...
use crate::{compose, gaussian_blur, unsharp};
...
pub fn drawing(buffer: &PixelBuffer<Rgb>, radius: u32, mode: DrawingMode) -> PixelBuffer<Rgb> {
    ...
    let drawing = despeckle(&drawing, 2);
...
fn despeckle(img: &PixelBuffer<Rgb>, amount: u32) -> PixelBuffer<Rgb> {
    img.map_colors_enumerated(|x, y, c| {
        if c.to_gray_with_intensity(Intensity::Average).red() < 0.05 {
            ...
            for y in 0..3 {
                for x in 0..3 {
                    if k[y][x].to_gray_with_intensity(Intensity::Average).red() < 0.05 {
                        count += 1;
                    }
                }
            }
            ...
            if count <= amount {
                ...
                for y in 0..3 {
                    for x in 0..3 {
                        let kc = k[y][x];
                        if kc.to_gray_with_intensity(Intensity::Average).red() >= 0.05 {
                            data[0] += kc.data[0];
                            data[1] += kc.data[1];
                            data[2] += kc.data[2];
                        }
